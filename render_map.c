...
/*   Updated: 2025/03/31 19:49:11 by imchaibi         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	load_textures(t_long *lng)
{
	...
	lng->collectible->img = mlx_xpm_file_to_image(lng->mlx,
	...
	lng->floor->img = mlx_xpm_file_to_image(lng->mlx,
...
void	render_tile(t_long *lng, int x, int y)
{
	...
	if (lng->map[y][x] == '1' && lng->wall
			&& lng->wall->img)
		mlx_put_image_to_window(lng->mlx, lng->win,
			lng->wall->img, pos_x, pos_y);
	else if (lng->map[y][x] == 'P')
		mlx_put_image_to_window(lng->mlx, lng->win,
			lng->player->img, pos_x, pos_y);
	else if (lng->map[y][x] == 'C')
		mlx_put_image_to_window(lng->mlx, lng->win,
			lng->collectible->img, pos_x, pos_y);
	else if (lng->map[y][x] == 'E')
		mlx_put_image_to_window(lng->mlx, lng->win,
			lng->exit->img, pos_x, pos_y);
}
